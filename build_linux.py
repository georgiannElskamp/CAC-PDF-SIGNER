...
from build_standalone import ROOT, licenses
...
def build(output, prefix, sources, work):
    ...
    command = [sys.executable, "-B", "-m", "PyInstaller", "--noconfirm", "--clean",
               "--onefile", "--console", "--noupx", "--name", "cac-signer",
               "--distpath", str(runtime), "--workpath", str(work / "freeze"),
...
def write_manifest(runtime):
    ...
    if any(name.lower().endswith((".dll", ".exe")) for name in archive.toc):
        raise ValueError("A Windows binary was included in the Linux worker.")
    modules = [p for p in ROOT.glob("*.py") if not p.name.startswith(("build_", "audit_", "publish_", "run_tests"))]
    manifest = {"version": VERSION, "platform": "linux", "architecture": platform.machine(),
                "sha256": hashlib.sha256(executable.read_bytes()).hexdigest(),
                "runtimeSources": {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(modules)},
                "nativeLibraries": sorted(n for n in archive.toc if ".so" in n or n == "pcscd")}
