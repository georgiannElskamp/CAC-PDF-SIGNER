...
For the ONLYOFFICE form-box work in PR #19, a saved form made by ONLYOFFICE 9.4.0.129 was inspected without changing the original. The Preview-mode click adapter was exercised in a disposable editor profile. A copy of the form was signed with an in-memory synthetic certificate and reopened with the visible appearance in the same rectangle; pyHanko verified its cryptographic integrity. A second form test confirmed that signature boxes remain signable while an ordinary Image field stays unchanged. Source-hash checks reject files changed after plugin startup before card access. The bundled Windows/Linux candidate passed packaged-worker preflight, and the maintainer reports successful physical CAC validation of that local candidate on 2026-09-23. The manual card check does not establish Linux editor behavior or preservation of every kind of multi-field ONLYOFFICE form. Hosted qualification is recorded in the PR checks.
