...
Monthly-maintenance tests cover the calendar, deadline at every API write, leap years, durable budgets, PR-only reconciliation, full-pass report evidence, workflow permissions, current-commit Codex evidence and rejection of unsafe dependency proposals. The private discovery workflow's manual dry-run also exercises the real Dependabot engine against the repositories and synthetic old pins. Branch dry-runs and updater jobs have no write credentials; Python metadata resolution has no repository or registry credentials. Full package builds remain required after dependency pins change.
